...
impl PopContext<'_> {
    ...
    pub unsafe fn buffer_unsafe(&self) -> BytesUnsafe {
        let mut len1 = 0usize;
        let mut len2 = 0usize;
        unsafe {
            let p1 = ffi::kz_pop_buffer(&self.raw, 0, &mut len1) as _;
            let p2 = ffi::kz_pop_buffer(&self.raw, 1, &mut len2) as _;
            BytesUnsafe::new(p1, len1, p2, len2)
        }
    }
