...
fn merge_inner(
    ...
    match (value, other) {
        ...
        (Value::Integer(_), Value::Integer(inner)) => {
            Ok(Value::Integer(inner))
        }
        (Value::Float(_), Value::Float(inner)) => Ok(Value::Float(inner)),
        (Value::Boolean(_), Value::Boolean(inner)) => {
            Ok(Value::Boolean(inner))
        }
        (Value::Datetime(_), Value::Datetime(inner)) => {
            Ok(Value::Datetime(inner))
        }
        (Value::Array(mut existing), Value::Array(inner)) => {
            if let Some(first) = inner.first() {
                if let Some("+") = first.as_str() {
                    existing.extend(inner.into_iter().skip(1));
                }
            }
